...
const fetchAll = async () => {
  loading.value = true;
  rows.value = [];
  let nextUrl = `${API}entidades/?page_size=100`;
  const p = paramsFromRoute();
  if (p.classificacao) nextUrl += `&classificacao=${encodeURIComponent(p.classificacao)}`;
  if (p.categoria)     nextUrl += `&categoria=${encodeURIComponent(p.categoria)}`;
  if (p.bairro)        nextUrl += `&bairro=${encodeURIComponent(p.bairro)}`;
  if (p.search)        nextUrl += `&search=${encodeURIComponent(p.search)}`;
